...
struct GovernmentStackTests {
    ...
    @Test func collapsedStackShowsAFullHeroAndPeekingRest() throws {
        ...
        #expect(abs(try cardHeight(controller, item: 1) - 56) < 1)  // peek: 56pt header
        #expect(abs(try cardHeight(controller, item: 2) - 56) < 1)
    }
    ...
    @Test func restingStateResetsWhenTheGroupDropsBelowTwoCards() throws {
        ...
        RunLoop.main.run(until: Date().addingTimeInterval(0.2))
        window.layoutIfNeeded()
        #expect(abs(try cardHeight(controller, item: 1) - 56) < 1)  // collapsed again
    }
}
