...
# 十三、皮夾裡的卡沒有名字（2026-08-16）

M5.5 把卡片列畫上首頁之後 render 出來看，第一列長這樣：

```
政府皮夾裡的卡
00000000_demo_drivinglicense_202504251418 · 有效到 2035年10月7日
```

四十一個字元的機器識別字串，在 390pt 寬的螢幕上折成兩行。
這不是排版沒調好，**這是憑證裡真的只有這個**。

## A. 事實

一張 TWDIW 憑證裡，唯一能拿來稱呼它的欄位是 `vc.type[1]`，
而它是一個發行系統的內部識別碼。人看得懂的名稱在別的地方：
發行者 metadata 的 `credential_configurations_supported[…].display[].name`——
那是一個 **HTTPS 端點**（實測 701 KB、882 組設定）。

於是：

> **一個離線的皮夾，無法告訴持有人他手上這張卡叫什麼。**

這跟撤銷那條不一樣。撤銷至少還有「離線超過 24 小時就不可知」這種
可以講清楚的降級；名稱是**第一次拿到卡之後就永遠只能靠當時有沒有連線**，
而且沒有任何機制把它跟憑證綁在一起——沒有簽章覆蓋它，
所以就算皮夾自己快取了，那份快取也不是憑證的一部分。

實務後果很具體：**在櫃檯前打開皮夾，畫面上是一串底線和數字。**
持有人要自己認出哪一張是駕照。

## B. 我們怎麼處理（以及刻意不做什麼）

`CardInventory.readableType` 只做一件很小的、不需要猜的事：
拿掉開頭全是數字的發行者代碼，拿掉結尾 8–14 位數字的核發時間戳，
其餘原樣輸出。上面那一列因此變成 `demo_drivinglicense`。

**刻意不做對照表。** 一張「已知型別 → 中文名稱」的表，
對表上沒有的型別就是一個猜測，而沒在表上的型別正是猜錯最要命的地方。
兩端的裁切都是條件式的，形狀不符就原樣輸出；裁完會變空字串也原樣輸出
（`00000000__202504251418` 就是這種，測試釘住了）。

這改善了可讀性，但**沒有解決問題**——`demo_drivinglicense` 依然不是中文，
依然是發行系統的字。真正的解法在發行端，所以它進回報清單。

## C. 進回報清單（第八件）

前七件在 §十.F。這是第八件：

> **憑證沒有攜帶任何離線可讀的名稱。** `vc.type[1]` 是內部識別碼，
> 人類可讀名稱只存在於線上 metadata，且不在簽章覆蓋範圍內。
> 建議：把 display name 放進 VC 本身（例如 `vc.name`，
> 或 SD-JWT VC 的 `vct` 搭配一份簽章覆蓋的型別 metadata），
> 使離線皮夾能顯示持有人認得的名稱。

受害程度排序上它不高——沒有人因此被冒用——
但它是**每一次打開皮夾都會遇到**的那種，而前七件多半不會。

## D. 同一次「把畫面 render 出來看」還抓到一個純粹的自家 bug

首頁那列「備份我的身分證件」的副標，原本依「有沒有任何卡」決定要說
「透過 MyData 服務建立」還是「重新抓一次，取代目前存的這份」。
放進一張政府卡、沒有自發行證件時，它說了後者——
**要取代一份根本不存在的東西**。判斷條件改成「有沒有*我們自己的*那一份」。

兩個都是 render 出來看才看到的。跟 `Item` 少了圖示那次一樣，
這一類缺陷沒有別的發現方式。
